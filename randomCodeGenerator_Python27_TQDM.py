...
#This version is Python 2.7, and requires TQDM.
...
#We need to import one thing to the program can generate random numbers. And to have a progress meter.
import random
import tqdm
...
permanentDigits = int(raw_input()) #The program asks for input, turns it into an integer/floating point, and stores it into a variable called "permanentDigits".
...
for number in tqdm.tqdm(range(permanentDigits)): #For every number in every number in 0-permanentDigits,
